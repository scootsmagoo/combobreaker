...
It is intentionally **not** a one-for-one replacement for heavyweights like uBlock Origin, Dark Reader, or Vimium â€” those are huge, ongoing projects. ComboBreaker covers the simpler 80% of what most of those extensions do, in a single auditable codebase.
...
| **Color picker** | Native `EyeDropper` API â€” one click, hex copied to clipboard. |
...
| **JSON formatter** | Auto-pretty-prints `application/json` responses. Tree / Formatted / Raw view modes, depth-level expand buttons (1â€“5, All), hover for JSON path + click to pin & copy, per-node "expand all descendants", auto/dark/light theme, and `window.data` exposed in the page console. |
| **Full-page screenshot** | Scroll-and-stitch the entire page to a PNG download. |
| **Video downloader** | A "Media" tab in the popup lists every `<video>`/`<audio>` element and every media response the page fetches (mp4, webm, mov, mp3, m4a, HLS `.m3u8`). One click downloads direct files via `chrome.downloads`. HLS streams open a dedicated downloader page that pulls every segment and saves a `.ts` file (run `ffmpeg -i input.ts -c copy output.mp4` if you want MP4). YouTube and other DRM/signed-stream sites get a "Copy yt-dlp command" button instead â€” see caveats below. |
| **Structured data / JSON-LD** | Extract, analyze, and build JSON-LD, microdata, and RDFa structured data. Includes validation links and a schema builder for creating new markup. |
| **Encoding override** | Manually set character encoding for legacy/garbled pages. |
| **Utility belt** | Tucked into the Tools tab â€” JWT decoder, encoder/decoder (Base64 / Base64-URL / URL / hex / HTML entity), regex tester with live highlights, Unix-timestamp â†” ISO-date converter, color converter (hex / rgb / hsl / oklch) with WCAG contrast checker, line/word diff viewer, fake-data + lorem-ipsum generator, password / UUID generator, and a locally-rendered QR code for the current URL. Each tool is a `<details>` collapsible â€” open only what you need. Everything is client-side; no network calls. |
...
4. Click **Load unpacked** (top left) and select the repo folder â€” the one containing `manifest.json`, not a parent or subfolder.
5. The extension appears in your list as "ComboBreaker." If the manifest has an error, Chrome shows it in red â€” click **Errors** for details.
...
- **Reload the extension** at `chrome://extensions` â†’ click the circular reload arrow on the ComboBreaker card. (Or click **Update** at the top of the page to reload all unpacked extensions at once.)
- **Reload affected tabs** â€” content scripts only inject on page load, so existing tabs keep running the old code until refreshed.
...
- **Popup**: right-click the toolbar icon â†’ **Inspect popup**. Opens DevTools scoped to the popup.
- **Service worker** (`background/`): on the `chrome://extensions` card, click the **service worker** link to open DevTools for it. MV3 service workers go idle after ~30s â€” interact with the extension to wake it back up.
...
- **Options page**: right-click the extension icon â†’ **Options**, then DevTools as normal.
...
The Media tab in the popup shows everything ComboBreaker has noticed on the current tab â€” the popup combines two sources, deduped by URL:
...
- **Direct files** (`mp4`, `webm`, `mov`, `mkv`, `mp3`, `m4a`, etc.) â€” one click downloads via `chrome.downloads.download` into a `combobreaker/` subfolder.
- **HLS** (`.m3u8`) â€” opens `viewer/hls_downloader.html` in a new tab. That page parses the manifest, lets you pick a variant if it's a master playlist, then fetches every segment and writes the concatenated MPEG-TS bytes to disk as a single `.ts` file. Works in VLC/mpv as-is. To convert to MP4 without re-encoding: `ffmpeg -i input.ts -c copy output.mp4`.
- **DASH** (`.mpd`) â€” detection only; no built-in downloader (use yt-dlp).
- **Copy yt-dlp** â€” copies `yt-dlp "<current tab URL>"` to your clipboard. Use this for YouTube, Vimeo, and anything else that ships signed/DRM streams.
...
- **No fragmented-MP4 muxing.** HLS streams whose segments are `.m4s` (with an init segment via `#EXT-X-MAP`) are detected and refused â€” concatenation alone doesn't produce a valid file. A real muxer (ffmpeg.wasm) would be needed; not currently vendored.
...
Dark mode is global-by-default, with per-site overrides â€” same model as Dark Reader itself.

- **Global toggle** (popup â†’ "Dark mode (all sites)") flips dark mode on/off for every page that doesn't have an explicit override.
- **Per-site override** (popup â†’ "This site: Auto / On / Off") opts the current site in or out, regardless of the global toggle. "Auto" means "follow global."
- **Tuning** (popup â†’ "Tune brightness / contrast â†’" or options page â†’ "Dark mode" section) exposes Dark Reader's sliders: mode (dark/light), brightness, contrast, sepia, grayscale. Changes apply live to every open dark-mode page.
- **Keyboard shortcut** (`Alt+Shift+D`) toggles the per-site override for the current site, matching Dark Reader's behavior â€” if dark is currently visible, it sets the site to "off"; if not, "on".
...
- Stores per-site settings in `chrome.storage.sync` (your encrypted Google profile, not our server â€” we don't have one).
...
PRs welcome. The whole point is auditability â€” please keep the codebase vanilla JS, no build step.
...
- **Dark Reader** ([darkreader.org](https://darkreader.org/), MIT) â€” powers the dark-mode engine. Vendored at `vendor/darkreader.js`.
- **qrcode-generator** by Kazuhiko Arase ([github](https://github.com/kazuhikoarase/qrcode-generator), MIT) â€” used by the QR utility. Vendored at `vendor/qrcode-generator.js`.
