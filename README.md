...
| **Link select** | Hold **Z** and drag (or press `Alt+Shift+L`, then drag: better on a trackpad) a box over a group of links to open them all in background tabs, open them in a new window, or copy them. Smart select takes just the headlines. The Linkclump idea. [Details ↓](#link-select) |
...
Hold the trigger and drag a box over links; let go and they all open in background tabs right after the current one, in page order. The trigger is **Z + drag** by default; Options → **Link select** offers Shift + drag, Alt + drag and right-button drag, and picks what happens on release (tabs, new window, or copy as URLs / title-tab-URL / a Markdown list). While dragging, **T**, **W** and **C** switch between tabs, window and copy for that one drag, **S** turns smart select on or off, **Esc** cancels, and holding the pointer at an edge scrolls the page so the box can cover more than a screen. **Smart select**: when the box holds both headline links (inside a heading, or bold) and ordinary ones, only the headlines are taken, so a news front page or search results give you the stories without their "comments" and "share" links. Duplicates, `javascript:` / `mailto:` links and links to a spot on the same page are dropped; more than 25 links asks first and 100 is the cap. While Z is held the cursor becomes a crosshair and a small "Link select" pill appears, and the popup's **Browse** tab says whether link select is running in the current tab (it is not in tabs that were open before the extension was loaded or updated, until they are refreshed, nor on Chrome's own pages and the Web Store). **On a trackpad**, holding a key while dragging is unreliable (confirmed on a MacBook trackpad, where Z + drag does nothing; Linkclump has the same limitation), so there is a start with nothing to hold: press **Alt+Shift+L**, or **Select links now** in the popup's Browse tab, and the next plain drag selects links (one selection, Esc cancels). With the right-button trigger on macOS and Linux the context menu needs a second right-click, because those systems open it on press rather than release.
...
| `Alt+Shift+L` | Link select without holding a key: the next drag selects links. For trackpads. |
...
- Three permissions are **optional** and only asked for when you first use the feature: `nativeMessaging` (the Allow step on the Helper setup page; only used for the ComboBreaker Helper, which you install yourself and which only ever runs its local `yt-dlp`), `contentSettings` (the first time you flip **Allow JavaScript**) and `notifications` (Options → Downloads → **Notify me when a download finishes**).
...
- `notifications`, once allowed, is used for finished or failed Helper downloads and the optional nuke shortcut.
...
The standalone policy is [`docs/PRIVACY.md`](docs/PRIVACY.md). Publishing to the Chrome Web Store: a step-by-step checklist, the listing text, single-purpose statement, a justification for every permission and the known review risks are in [`docs/STORE_LISTING.md`](docs/STORE_LISTING.md); the listing screenshots in `docs/store/` are regenerated with `node scripts/store_screenshots.js`.
