...
// Site keys have "www." stripped; this returns the hosts a key stands for.
// IPs and single-label hosts (localhost) have no "www." sibling — Chrome
// rejects "www.127.0.0.1" as a match pattern / origin.
export function hostsForSite(siteKey) {
  if (!siteKey || siteKey === "file://") return [];
  const isName = siteKey.includes(".") && !/^[\d.]+$/.test(siteKey) && !siteKey.includes(":");
  return isName ? [siteKey, `www.${siteKey}`] : [siteKey];
}
