...
// Human name for a static block rule: "||doubleclick.net^" -> "doubleclick.net".
export function ruleLabel(rule) {
  const c = (rule && rule.condition) || {};
  if (typeof c.urlFilter === "string") {
    const m = c.urlFilter.match(/^\|\|([a-z0-9.-]+)/i);
    return m ? m[1] : c.urlFilter;
  }
  if (Array.isArray(c.requestDomains) && c.requestDomains.length === 1) return c.requestDomains[0];
